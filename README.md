...
O [modelo CSV](apps/admin/public/modelo-produtos.csv) contém apenas o cabeçalho, sem produtos fictícios. Preencha uma linha por item com `sku`, `descricao`, `departamento` e `unidade_venda`. As colunas `secao`, `grupo` e `subgrupo` são opcionais nesta prévia; arquivos feitos com o cabeçalho anterior continuam aceitos. Os demais campos ajudam a preparar preço, custo e revisão fiscal. Abra a página **Produtos → Preparar lista de produtos** para baixar o modelo e validar o arquivo. A prévia roda no navegador e não grava dados. Ela identifica campos obrigatórios, SKU e GTIN repetidos, dígito verificador de GTIN, formato de NCM/CEST e valores monetários. Colunas adicionais, inclusive de tributação, aparecem como **ainda sem mapeamento** e não são validadas. Quando a lista real chegar, os campos tributários serão mapeados conforme a planilha recebida. Código NCM, incidência tributária e preço exigem conferência específica antes de liberar o produto para venda.
